...
Scenario::Scenario(const std::vector<std::string>& word_list, std::size_t nqueries)
  : m_impl(std::make_unique<scenario_impl_t>())
{
  std::random_device rd;
  std::mt19937 gen(rd());
  ...
  this->m_impl->init_word_list = std::vector<std::string>(search_list, insert_list);
...
std::vector<result_t> Scenario::execute(IDictionary& dic) const
{
  ...
  for (auto&& q : this->m_impl->queries)
  {
    switch (q.op)
    {
    case query::search: results.push_back(dic.search(q.arg)); break;
    case query::insert: dic.insert(q.arg); break;
    case query::erase:  dic.erase(q.arg); break;
    }
  }
...
void Scenario::execute_verbose(IDictionary& dic) const
{
  ...
  for (auto&& q : this->m_impl->queries)
  {
    ...
    case query::search:
    {
      ...
      std::cout << "S: " << q.arg << " -> " << "(" << r.first << ", " << r.second << ")\n";
      break;
    }
    case query::insert:
      std::cout << "A: " << q.arg << "\n";
